...
<html lang="en">
	...
	<body>
							...
							<div class="header-search" >
								<form method="POST">
									...
									<select type="text" id="categoria" name="categoria" class="input-select">
										...
										<option value="N°Compra">N°Compra</option>
		...
		<div id="content">
			<br>
			<h2 style="text-align: center;">Pagos registrados en el sistema</h2>
			<br>
			<br>
					...
					<tbody>
						@foreach ($payments as $payment)
							...
							@foreach ($users as $user)
								@if ($user->id == $payment->id_user)
									<td>{{$user->cedula}}</td>
								@endif
							@endforeach
							@foreach ($products as $product)
								@if($product->id == $payment->id_product)
							...
							<td>
								<a class="boton-admin-table" href="{{url('dpayment',Crypt::encrypt($payment->id))}}"><i class="fa fa-trash"></i></a>
							</td>									
							</tr>
						@endforeach
					</tbody>
